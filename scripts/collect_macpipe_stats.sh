...
Ex50N50=$( grep -E "^51" $ExN50_f | awk '{print $2}' )
Ex90N50=$( grep -E "^92" $ExN50_f | awk '{print $2}' )
...
busco_stats=$( head -n 8 $busco_f | tail -n 1 |  sed -E 's/C:([0-9.]+)%\[S:([0-9.]+)%,D:([0-9.]+)%\],F:([0-9.]+)%,M:([0-9.]+)%,n:([0-9.]+)/\1\t\2\t\3\t\4\t\5\t\6/')
...
busco_stats=$busco_stats"\t"$busco_stats_trans

# transrate
transrate_f="transrate/summary/"$1".assemblies.csv"
transrate_stats=$( tail -n 1 $transrate_f | awk -F "," '{$1=""; print $0}' )
transrate_header=$( head -n 1 $transrate_f | awk -F "," '{$1=""; print $0}' )
...
transdecoder_l=$( awk '{print $NF}' $transdecoder_fp | sort -n | awk '{if(min==""){min=max=$1}; if($1>max) {max=$1}; if($1< min) {min=$1}; total+=$1; count+=1} END {print total/count, min, max}' )
transdecoder_c=$( wc -l "transdecoder/"$1"_longest_orfs.pep" | awk '{print $1}' )
...
mapping_stats=$( tail -n 1 $mapping_f | awk '{print $1"\t"$2"\t"$4"\t"$6"\t"$8}' )
